...
| **终端风首页** | v1.2.6 落地页整体终端化：黑底 + 等宽字体 + 扫描线/噪点；导航按参考图（文字品牌 + 时钟 + ARCHIVE.FS + 登录）；Hero 左对齐新标题"一切终将归档"；功能表 / 联系方式 / BGM 控件均为终端风格，纯黑白灰 |
...
- **默认背景音乐**：主页 / 登录注册页 / 后台系统默认播放电机背景音乐，支持播放 / 暂停 / 静音，页面跳转不中断，与音乐盒互斥；v1.2.6 控件改为终端风（方形描边 + 字符图标 + BGM 状态标签）
...
│   │       └── default-bgm.tsx       #     默认背景音乐播放器（终端风，v1.2.6）
│   │
│   ├── hooks/                         # 🔹 自定义 Hooks
