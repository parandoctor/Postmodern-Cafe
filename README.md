...
**后现代咖啡馆 (Rainbow-box)** 是一个基于 Next.js 的综合服务平台，在个人文件收纳能力之上，进一步覆盖生活记录、资料归档与事务处理。它采用"白、浅灰、中灰、灰、深灰、墨、黑"七级黑白灰阶作为分类体系，以 Notion 风格统一后台界面，让生活与事务管理变得直观、优雅且高效。支持拖拽上传、在线预览、智能搜索、收藏、回收站等文件能力，并提供任务管理（今日任务）、随手记、音乐盒、日历与计时器等效率面板。

当前版本 **v1.2.1** 完成随时记写入口页面化：点击侧边栏"随时记写"跳转独立工作台，全文在中间操作台呈现，长文本自动换行不影响观览；每日待办合并入任务管理构成长期工作流，每日完成的任务由空圆圈变为实心圆圈；任务管理支持任务名、目的、关联文件与知识、完成状态、三档重要性、父子任务多级拆分，以及列表 / 时间线两种视图与拖拽自定义排序。
...
- **效率面板**：左侧今日任务（调用任务管理数据）、随时记写、音乐盒（本地音乐上传播放）；右侧日历与计时器；随时记写点击后进入独立工作台页面，全文在中间操作台呈现
...
│   │   │   ├── sidebar-todo.tsx       #     今日任务（v1.2.1 起调用任务管理 API）
