# <img src="public/images/cafe-logo-black.png" width="26" height="26" align="center" alt="后现代咖啡馆"> 后现代咖啡馆 (Postmodern Cafe) `v1.2.8`
...
当前版本 **v1.2.8** 首页双栏落地：左侧终端内容左对齐收窄，右侧新增「剪影摄影档案图」（黑白剪影，档案边框 + FIG 标注，纯黑白灰）；登录/注册页卡片高透明、背景清晰，柔和玻璃按钮（鼠标远离半透明、靠近变白底 + 光晕 + 上浮 + 扫描光），并修复输入框失焦、BGM 连贯播放与登录注册布局。此前 v1.2.6 首页设计创新：落地页整体改造为黑底终端档案风——顶部导航（左侧文字品牌 + 实时时钟 / ARCHIVE.FS / 白色描边登录）、左对齐终端 Hero（ASCII 字标 + 终端命令 + glitch 标题"一切终将归档"）、终端档案表式功能介绍、终端频道列表式联系方式，并配两张黑白艺术图（构成主义几何 / 点阵人像）；右下角 BGM 控件同步终端化。此前 v1.2.5 新增电机背景音乐：主页、登录注册页与后台系统默认播放，右下角提供播放 / 暂停 / 静音控制，页面跳转不中断，与音乐盒互斥播放。此前 v1.2.4 将侧边栏"我的文件 / 分类管理 / 收藏夹 / 最近使用 / 回收站"五个文件功能入口合并为"文件管理"：选中文件管理后，中间操作台上方显示五个分栏用于切换，默认主页为分类管理；v1.2.3 为音乐盒操作台引入千禧风 Y2K 复古播放器（参考小红书"沐果素材"Y2K 播放器素材包的视觉语言，纯 CSS 实现不引入版权素材）：Winamp 风格银色金属机身 + 黑白灰 LCD 频谱屏 + 可拖拽进度/音量滑块 + 随机/单曲循环 + 旋转 CD 与磁带装饰，右侧为 Winamp 风格播放列表编辑器，整体为黑白灰单色；v1.2.2 统一私有空间（侧边栏"工作区 / 私有空间"同级并列，去掉私有空间外框，今日任务、随时记写、音乐盒均可点击标题在中间操作台全屏打开独立工作台）。
...
| **终端风首页** | v1.2.6 落地页整体终端化：黑底 + 等宽字体 + 扫描线/噪点；导航按参考图（文字品牌 + 时钟 + ARCHIVE.FS + 登录）；Hero 左对齐新标题"一切终将归档"；功能表 / 联系方式 / BGM 控件均为终端风格，纯黑白灰；v1.2.8 改为左右双栏，右侧新增剪影摄影档案图 |
...
- **终端档案风首页**（v1.2.6）：落地页整体黑底终端化——ASCII 字标、终端命令、glitch 标题、档案表 / 频道列表式模块、扫描线 + 噪点肌理，纯黑白灰
...
- **默认背景音乐**：主页 / 登录注册页 / 后台系统默认播放电机背景音乐，支持播放 / 暂停 / 静音，页面跳转不中断，与音乐盒互斥；v1.2.6 控件改为终端风（方形描边 + 字符图标 + BGM 状态标签）；v1.2.8 重构为模块级单例 Audio，进入页面自动尝试播放、切换页面不中断、整页刷新后从上次进度续播
...
├── public/                            # 静态资源
│   ├── images/                        # 公共图片
│   ├── art/                            # 首页终端风黑白艺术配图（v1.2.6 / v1.2.8 剪影档案图）
│   ├── audio/                          # 默认背景音乐
...
│   │   │   └── task-card.tsx          #     任务卡片
│   │   ├── landing/                   #   首页组件
│   │   │   ├── navbar.tsx             #     终端风导航（文字品牌 + 时钟 + ARCHIVE.FS + 登录，v1.2.6）
│   │   │   ├── hero.tsx               #     左对齐终端 Hero（ASCII 字标 + 终端命令 + glitch 标题，v1.2.6）
...
│   │       ├── entrance.tsx           #     滚动入场动画封装（v1.2.6）
│   │       └── default-bgm.tsx       #     默认背景音乐播放器（单例 + 续播，v1.2.6 / v1.2.8）
│   │
│   ├── hooks/                         # 🔹 自定义 Hooks
